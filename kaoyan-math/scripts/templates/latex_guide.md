...
| $\vert \vert$ | `\vert \vert` | 绝对值（推荐） |
| $\| \|$ | `\| \|` | 范数 |

> [!warning] ⚠️ 表格中绝对值符号规范
> **在 Markdown 表格中，必须使用 `\vert` 代替 `|`！**
>
> 原因：`|` 是 Markdown 表格的列分隔符，会导致表格结构错乱。
>
> | 场景 | 正确写法 | 错误写法 |
> |:---|:---|:---|
> | 表格内 | `$\vert f(x) \vert$` | `$|f(x)|$` ❌ |
> | 普通段落 | `$|f(x)|$` 或 `$\vert f(x) \vert$` | 都可以 |
> | Callout 块内 | `$|f(x)|$` 或 `$\vert f(x) \vert$` | 都可以 |
>
> **示例对比**：
> ```markdown
> # ❌ 错误 - 表格会被破坏
...
>
> **其他替代写法**：
> - `\lvert ... \rvert`（左右绝对值符号，语义更清晰）
> - `\left| ... \right|`（自适应大小，但仍有表格问题）
> - `\left\vert ... \right\vert`（自适应大小 + 表格安全）
